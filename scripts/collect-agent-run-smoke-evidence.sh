...
The collector proceeds only when \`pre-capabilities.json\` advertises \`run\`, the \`test\` workload, and the safe \`runLifecycle\` contract used by guarded wrappers, then \`pre-diagnose.json\` reports \`safeToRequestCooling=true\`, \`daemonControlPathReady=true\`, and \`recommendedAgentAction\` is either \`requestCooling\` or \`requestCoolingWithCaution\`. The caution path is still bounded smoke evidence; do not raise duration or RPM just because the collector proceeds.
...
if [[ -n "${skip_reason}" ]]; then
  ...
  write_summary_json "blocked" "true" "false" "" "${skip_reason}"
  write_checksums
