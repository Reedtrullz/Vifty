...
3. read-only `viftyctl capabilities --json`, require any nonzero exit to match the advertised unavailable exit code, require advertised `run` command support, require the requested workload name, require the safe `runLifecycle` contract used by `viftyctl run`, require `metadataLimits`, and reject reasons longer than the advertised maximum before readiness or cooling;
...
If a capabilities payload does not advertise `runLifecycle` or `metadataLimits`,
treat those guarantees as unavailable and refuse cooling.
