...
6. fail closed when readiness is blocked, `safeToRequestCooling` is false, `daemonControlPathReady` is false, `manualControlActive` is true, `daemonRuntime.matchRequired` is true while `daemonRuntime.matchesExpectedDaemon` is not true, or `coolingBlockerIDs` is non-empty, and print `recoverySteps` guidance for helper repair, Auto restore, workload backoff, policy inspection, or hardware-evidence follow-up. When diagnose includes display-only `operatorRecoveryCommands`, including helper repair or Restore Auto commands tied to the current app bundle, agents must honor `safeForAgentsToRunAutomatically: false`, show them to a human instead of running them, and preserve them only as display-only data in guarded-run decision JSON. Do not loop `restore-auto`; if `manualControlActive` stays true after one restore, inspect `appPreferences.startupMode`, then stop and ask the user to switch Vifty/default startup mode to Auto. When diagnose reports a persisted `Curve` or `Fixed` default, the wrapper also prints that saved default in plain stderr before the JSON;
