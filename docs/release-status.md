...
As of 2026-07-13, `v1.3.2` is the current published Developer ID release. Its immutable tag resolves to `6a771c2ea10386bf7a0a8369a759930f01d56062`, source CI run `29284751837` passed, signed/notarized Release run `29285576026` passed, and the four canonical trust assets are public at the [v1.3.2 GitHub Release](https://github.com/Reedtrullz/Vifty/releases/tag/v1.3.2). `v1.1.1` remains the published source-first fallback; its immutable tag resolves to `a82f2237ff39c24a6b366dca8f95a17ee54fd972`.
...
Installed release-mode validation currently remains recorded only for the exact public `v1.3.0` build `5`. That historical evidence includes matching signed helper parity and a ready Auto state, but it does not transfer to `v1.3.2` build `7`. Exact-build helper parity, installed release-mode review, and supervised Fixed/Curve/Auto smoke must be repeated before those claims are promoted for `v1.3.2`.
...
- Independent verification of the downloaded public `v1.3.2` artifact passed. No installed release-mode, signed-helper parity, Auto-restoration, or manual Fixed/Curve compatibility claim is made for `v1.3.2` until exact-build evidence is reviewed.
