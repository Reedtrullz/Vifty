...
Installed release-mode validation for `v1.3.0` is not yet recorded because replacing the installed app/helper is a separate privileged, potentially fan-affecting operation. The latest installed evidence remains the exact public `v1.2.0` build `4`: after explicit human approval, Vifty restored both fans from Forced control to macOS Auto, cleared the manual-control marker, quit cleanly, and installed it in `/Applications`. That installed app and privileged helper are Developer ID signed by TeamID `X88J3853S2`, their daemon hashes match, the LaunchDaemon carries the same TeamID allowlist, and read-only diagnose reported `state: "ready"`, `manualControlActive: false`, no cooling blockers, and nominal thermal pressure.

The privacy-safe release bundle at `.build/public-release-v1.2.0/installed-release-evidence-public-safe` passed `scripts/review-validation-evidence.sh --mode release`. Its schema-backed `review-result.json` records `status: "passed"`, `readOnly: true`, and `coolingCommandsRun: false`, exact source tag/commit/artifact SHA, `installSource: "notarized-github-release"`, helper parity, and no failures or warnings. The safe reviewed summary is checked in at [validation-reports/2026-07-11-v1.2.0-macbookpro18-release/review-result.json](validation-reports/2026-07-11-v1.2.0-macbookpro18-release/review-result.json); the raw 100-file bundle remains ignored under `.build`. Manual Fixed/Curve compatibility smoke remains `not-recorded`; this release-install proof must not be promoted into a hardware compatibility claim.

Release lanes:

1. **Published Developer ID release:** `v1.3.0` public artifact and cask trust checks passed for the tagged workflow, canonical assets, checksum handoff, public verifier, release readiness, TeamID, notarization, stapling, and Gatekeeper. Installed app/helper parity and Auto-restoration evidence remains recorded only for `v1.2.0`; manual hardware compatibility evidence remains separate.
...
Auto-update status: unavailable in `v1.3.0`, source-first, and unsigned-dev builds. Vifty should use Sparkle only in a separately reviewed Developer ID signed/notarized release with signed appcast metadata; see [auto-update.md](auto-update.md).
...
- `scripts/check-release-readiness.sh --mode developer-id --version 1.3.0 --repo Reedtrullz/Vifty --require-source-ref 5cfd0eaf4409e3cfd2e57c766297d1609f8698cb --json` reported `ready` before the cask follow-up moved `main`.
- Independent verification of the downloaded public `v1.3.0` artifact passed. Installing it and collecting matching installed-app/helper evidence remains pending explicit approval; no `v1.3.0` fan-control or Auto-restoration claim is made here.
