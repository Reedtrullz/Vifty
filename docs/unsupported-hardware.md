...
Unsupported hardware safe block is expected behavior, not a bug to bypass. When a machine is outside the write-control scope, `viftyctl diagnose --json` should report `state: "blocked"` and `safeToRequestCooling: false`, and the app, helper, daemon, and local agent paths should refuse manual or agent cooling.
...
Local agents must treat `safeToRequestCooling: false`, `recommendedAgentAction: "doNotRequestCooling"`, and `state: "blocked"` as hard stops. They may continue the workload without Vifty cooling only when the user explicitly wants that.
