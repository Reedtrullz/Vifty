...
The source-build and verified public-archive installer lanes replace only the unprivileged app bundle. They do not stop, remove, repair, or overwrite the installed LaunchDaemon helper, and their replacement preflights are read-only.

The public-archive lane is an explicit operator bridge, not an app-controlled downloader. It begins with `v1.4.0`; the historical `v1.3.2` lifecycle has no root snapshot binding contract and is rejected. It accepts only an absolute path to the canonical archive selected from the reviewed checkout's single current `.github/release-manifest.json` `publishedRelease`; candidate and historical entries, direct app bundles, URLs, relative paths, and SHA overrides are outside the trust boundary. Before extraction, it requires the exact manifest-pinned SHA-256 and signed-tag/signer-policy verification. Bounded private extraction then yields a complete candidate content digest. Before replacement, a no-skip public verifier result and independent extracted-bundle checks must pass version/build, bundle identities, Developer ID TeamID, deep signature, notarization/stapling, and Gatekeeper. The privileged root snapshot independently matches the candidate digest before helper teardown. For an existing install, the same prepare record also binds the complete preflighted previous-app digest; the atomically isolated previous bundle must match it before installation or eventual transaction cleanup. A valid source-archive quarantine marker is propagated to the candidate, and the public copy path does not blanket-clear candidate extended attributes; these metadata are not authorization and Gatekeeper is still assessed on each copy. A private destination lock, authenticated downgrade refusal, and no second-destination fallback remain in force through completion. The advisory update checker does not call this lane, and this manual bridge is not Sparkle or silent in-place update.
...
The generated fact block above is authoritative for the current public version, build, architecture, identities, checksum, TeamID, and trust state. The exact `v1.4.8` public artifact passes release-level signing/notarization checks, installed release-mode review, and supervised Fixed → Temperature Curve → Auto validation on `MacBookPro18,1`; the [release review](validation-reports/2026-09-13-v1.4.8-macbookpro18-release/review-result.json) and [manual-smoke attestation](validation-reports/2026-09-13-v1.4.8-macbookpro18-supported/manual-smoke-attestation.md) scope those claims to that exact binary and model. The historical `v1.3.2` report remains valid for that older binary only and does not validate the current branch or broad Apple Silicon compatibility. `v1.1.1` remains the source-first fallback and supersedes `v1.1.0` for users who hit the helper-unreachable update issue. Any `Vifty-v<version>-unsigned-dev.zip` attachment is not Developer ID signed, not notarized, not Homebrew-trusted, and must not use the canonical `Vifty-v<version>.zip` release artifact name.
