...
| `daemonRuntime.matchRequired: true` with `daemonRuntime.matchesExpectedDaemon` not `true` | Do not request cooling. Repair or reinstall the helper so the installed LaunchDaemon matches the `viftyctl` build before retrying. If diagnose includes `operatorRecoveryCommands`, show them to the human only and honor `safeForAgentsToRunAutomatically: false`. |
| Diagnose `recommendedRecoveryAction: "repairHelper"` | Show payload-local `recoverySteps` and any display-only `operatorRecoveryCommands` when available, falling back to `repairHelperRecoveryActions` from `viftyctl agent-rule --json`: open Vifty and use Repair/Reinstall Helper, or in a source checkout run `make repair-helper`, then rerun `diagnose --json`. Do not attempt direct SMC writes, run recovery commands automatically, or use uncooled guarded fallback. |
| Diagnose `recommendedRecoveryAction: "restoreAutoBeforeRetry"` | Restore Auto once, re-run diagnose, clear manual/user ownership, or wait for the active lease to clear before retrying. If diagnose includes a display-only `restore-auto-current-app` operator command, show it to the human and honor `safeForAgentsToRunAutomatically: false`. |
...
Do not parse human-readable warning text when the JSON fields exist. Pin automation to `state`, `recommendedAgentAction`, `recommendedRecoveryAction`, `recoverySteps`, `safeToRequestCooling`, `daemonControlPathReady`, `manualControlActive`, `daemonRuntime`, `failedCheckIDs`, `coolingBlockerIDs`, `appPreferences.startupMode`, `checks`, and `agentControl`.
...
If a human chooses to run `/Applications/Vifty.app/Contents/MacOS/viftyctl restore-auto --json`, a successful command clears the same local `manualControlActive` marker that `diagnose --json` uses for the restore-first gate. `restore-auto` refuses to override an active manual fan-control session unless the human passes `--operator`; agents must never pass `--operator`. Always re-run `diagnose --json` after restore and require `manualControlActive: false`, `safeToRequestCooling: true`, and `daemonControlPathReady: true` before requesting cooling. Do not loop `restore-auto`; if `manualControlActive` stays true after one restore, inspect `appPreferences.startupMode`, then stop and ask the user to switch Vifty/default startup mode to Auto before another cooling request. Agents may display `restore-auto-current-app` from `operatorRecoveryCommands`, but must not execute it automatically. The user may also disable agent cooling entirely in Vifty Settings; when `policy.enabled` is `false`, treat cooling as disabled and refuse to request it.
