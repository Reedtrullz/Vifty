...
- `viftyctl diagnose` exits 75 for blocked readiness after printing structured JSON, while validation evidence collection and the guarded-run wrapper both preserve blocked reports correctly. Diagnose JSON also includes `recommendedAgentAction`, `recommendedRecoveryAction`, and `safeToRequestCooling` so agents can distinguish normal, caution, restore-first, stop, and safe recovery decisions without parsing warning text.
