...
**Status:** Active. Local hardening is on `main` for the `v1.1.0` source-first release, and the current local trust gate verifies 431 XCTest cases plus the release app bundle checks. Public binary trust still depends on a future Developer ID signed/notarized release, Homebrew release metadata, and real Apple Silicon MacBook Pro validation reports.
