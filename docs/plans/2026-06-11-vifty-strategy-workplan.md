...
- `scripts/check-release-readiness.sh` now gives maintainers a single read-only human/schema-backed JSON preflight for release metadata, optional required source-ref alignment, release-commit CI status, required secret names, and GitHub Release asset presence, so `v1.1.0` can be reported as source-ready but public-release blocked without ambiguity, and stale release tags can be rejected before promotion.
