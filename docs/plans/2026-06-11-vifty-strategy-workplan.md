...
- `guarded-run.sh` now has an explicit `VIFTY_GUARDED_RUN_ALLOW_UNCOOLED=1` path for the case where Vifty blocks cooling but the user wants the child workload to run normally anyway. It still runs read-only capabilities/readiness checks, prints diagnose JSON, refuses to request cooling, and then execs the child directly only for allowed recovery actions; it still refuses uncooled execution for `backOffWorkload` and `restoreAutoBeforeRetry`.
