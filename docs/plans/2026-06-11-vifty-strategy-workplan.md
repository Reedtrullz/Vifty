...
- `examples/viftyctl/guarded-run.sh` provides a shell-friendly preflight plus `viftyctl run --json` wrapper, and `examples/viftyctl/*` now includes tested shortcut scripts for Swift, Xcode, Make, npm, cargo, pytest, local model, and custom workloads, with XCTest coverage proving capabilities `run` command and requested-workload discovery, advertised unavailable-exit handling, `runLifecycle` contract checks, wrapper duration/RPM/reason preflight before contacting Vifty, `supportsForceRetry` checks before passing `--force`, required `recommendedRecoveryAction` handling, missing capability-field fail-closed behavior, ready/degraded/blocked behavior, restore-first decision blocking, missing-decision fail-closed behavior, diagnose failure preservation, opt-in `VIFTY_GUARDED_RUN_FORCE_RETRY` handling, missing and non-executable child-command preflight, structured pre-child failures, executable shortcut scripts, and no raw fan-control commands in those wrappers.
