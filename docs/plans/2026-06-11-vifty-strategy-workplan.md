...
- `scripts/review-agent-cooling-evidence.sh` reviews those lightweight agent/helper bundles, writes schema-backed `agent-cooling-evidence-review.json` with `schemaID: https://vifty.local/schemas/agent-cooling-evidence-review.schema.json`, accepts blocked diagnose exit `75` as evidence, records a `diagnoseDecision` summary for the diagnose exit status, readiness state, `recommendedAgentAction`, `recommendedRecoveryAction`, `safeToRequestCooling`, and `daemonControlPathReady`, records a `capabilitiesDecision` summary for advertised `viftyctl run` support, force-retry discovery, safe run/direct-control lifecycle, metadata limits, daemon status, and unavailable-exit metadata, and rejects schema drift, manifest/status/checksum drift, privacy findings, missing or contradictory diagnose decision fields, missing or unsafe capabilities contract fields, or any bundle that says cooling commands were run before maintainers treat the report as evidence.
