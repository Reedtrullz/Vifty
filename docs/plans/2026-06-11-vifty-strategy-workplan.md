...
- `scripts/collect-agent-cooling-evidence.sh` gives agent/helper reports a narrow read-only bundle path that captures `capabilities`, `diagnose`, `status`, and bounded `audit` JSON plus exit statuses, launchd/helper install evidence, a manifest, `privacy-review.tsv`, and checksums without requesting cooling, restoring Auto, calling `ViftyHelper`, using `sudo`, or writing SMC keys. Its privacy review flags likely hostnames, `/Users/...` paths, serial-number labels, and hardware UUID labels before reporters paste agent/helper evidence into public issues.
