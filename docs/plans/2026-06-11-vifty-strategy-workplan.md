...
- `viftyctl diagnose` exits 75 for blocked readiness after printing structured JSON, while validation evidence collection and the guarded-run wrapper both preserve blocked reports correctly. Diagnose JSON also includes `recommendedAgentAction`, `recommendedRecoveryAction`, `safeToRequestCooling`, and `daemonControlPathReady` so agents can distinguish normal, caution, restore-first, helper-repair, stop, and safe recovery decisions without parsing warning text.
