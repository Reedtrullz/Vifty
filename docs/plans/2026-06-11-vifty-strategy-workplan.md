...
**Status:** Active. Post-release local hardening is on `main` after the published `v1.1.0` source tag (`f7d2c636ebf582ac3809998c3fac819d5d87eb72`), and the current local trust gate verifies 446 XCTest cases plus community/support surface checks and release app bundle checks. Public binary trust still depends on a future Developer ID signed/notarized release, Homebrew release metadata, and real Apple Silicon MacBook Pro validation reports.
