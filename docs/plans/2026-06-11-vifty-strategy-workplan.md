...
- `scripts/summarize-validation-reports.sh` builds JSON/TSV indexes plus an optional conservative `compatibility-matrix.md` draft from valid reviewed `review-result.json` files, rejects missing or drifted validation-review-result schema IDs plus missing `daemonControlPathReady`, missing or unsupported `recommendedAgentAction`, missing or unsupported `recommendedRecoveryAction`, malformed, non-read-only review outputs, or release-mode rows with source-first/non-release install sources before they become claims, preserves install provenance, source checksums, `modelFamily`, agent decisions, recovery guidance, agent-run smoke evidence, and daemon control-path readiness in compatibility indexes, adds summary counts by model family, validated hardware reports by model family, agent action, recovery action, `safeToRequestCooling`, and daemon control-path readiness for fleet-level safety triage, keeps candidate supported-hardware rows labeled as manual-smoke-required until the review result records `manualSmokeTestResult: "passed-auto-restored"`, then promotes them to `validated-hardware-evidence`; the Markdown draft ignores release-trust rows for hardware status and keeps agent-run smoke separate from manual fan-control validation.
