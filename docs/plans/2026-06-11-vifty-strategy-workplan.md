...
- `scripts/review-agent-cooling-evidence.sh` reviews those lightweight agent/helper bundles, writes schema-backed `agent-cooling-evidence-review.json` with `schemaID: https://vifty.local/schemas/agent-cooling-evidence-review.schema.json`, accepts blocked diagnose exit `75` as evidence, and rejects schema drift, manifest/status/checksum drift, privacy findings, or any bundle that says cooling commands were run before maintainers treat the report as evidence.
