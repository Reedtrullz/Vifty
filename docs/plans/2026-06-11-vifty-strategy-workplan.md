...
**Status:** Active. Post-release local hardening is on `main` after the published `v1.1.1` source-first hotfix tag (`a82f2237ff39c24a6b366dca8f95a17ee54fd972`), and the current local trust gate is fast by default, while `make verify-full` and GitHub Actions carry the full XCTest suite plus community/support surface checks and release app bundle checks. Public binary trust still depends on a future Developer ID signed/notarized release, Homebrew release metadata, and real Apple Silicon MacBook Pro validation reports.

**Current public-release audit:** On 2026-06-11, the published `v1.0.0` GitHub asset existed and its asset digest matched the then-current `Casks/vifty.rb`, but `scripts/verify-release-artifact.sh` failed because the extracted app bundle reported `CFBundleShortVersionString` `0.1.0` while the cask version was `1.0.0`. Treat the `v1.0.0` public artifact as not trust-complete. The `v1.1.0` GitHub Release was source-first at source commit `f7d2c636ebf582ac3809998c3fac819d5d87eb72`, but it is superseded for public testers because it predates helper-install and app-polling hardening that fixed the helper-unreachable update issue. The current source-first hotfix release is `v1.1.1` at source commit `a82f2237ff39c24a6b366dca8f95a17ee54fd972` and attaches only `Vifty-v1.1.1-unsigned-dev.zip` plus `Vifty-v1.1.1-unsigned-dev.zip.sha256` as tester convenience assets. Later `main` commits are post-release hardening, not a silent retag. Do not promote Homebrew or a trusted binary install until Apple Developer Program credentials exist and the Developer ID release lane passes. [release-status.md](../release-status.md) is the current public status page for that distinction.
...
- Strong test posture: current local suite covers the audit remediation, release/readiness evidence schemas, validation report index schema, validation review-result schema, cask trust-metadata, cask checksum handoff, release secret/readiness/source-ref/source-CI/release-workflow/source-first preflights, source-first Source Provenance enforcement, unsigned-dev artifact source-ref guard checks, GitHub Actions Node.js 24 runtime opt-in and cache-action versioning, release status trust documentation, release checklist/source-first note publication, release asset-publication checks, release TeamID build wiring checks, release tag/version identity checks, release verifier skip-flag checks, release-summary consistency review checks, release-summary capture-time consistency checks, release-checklist evidence collection/review checks, validation install-provenance capture/review/index checks, validation privacy-review checks including generated summaries, validation capabilities-contract checks, direct validation TSV content checks, review-summary TSV/JSON consistency checks, collector manifest output/status checksum coverage checks, manifest status/output consistency checks, evidence-bundle checksum coverage/recomputation checks, daemon-unavailable capabilities review checks, review-result schema identity checks, malformed review-result indexer rejection checks, capabilities run-lifecycle metadata, guarded-run lifecycle contract checks, guarded-run wrapper-argument preflight checks, guarded-run advertised run-command/workload support checks, guarded-run advertised unavailable-exit checks, guarded-run force-retry capability checks, guarded-run readiness recovery-action checks, guarded-run null-field normalization, capability legacy-field fail-closed decoding, force-retry opt-in, interrupted force-retry JSON error reporting, command-error recovery-action metadata, command-error retry metadata, readiness recovery-action metadata, daemon control-path readiness evidence, missing option-value parse failures, child-command failure classification, guarded-run child executable preflight checks for PATH and direct paths, workload shortcut checks, real-process child-command resolution/signal-exit checks, non-directory executable validation checks, Auto-restore fan-shape/range preflight checks before SMC/XPC access, helper install/approve/repair action-copy checks, signal-handler restoration checks, fan-control ownership UI summary checks, developer workload preset checks, validation-report hardening, safe-agent-cooling guidance, restore-auto scoped-option rejection, support-triage, unsupported-hardware policy, GitHub topic/label metadata checks, community/support standard checks, PR safety review, and CODEOWNERS safety-surface checks.

### Vifty Weaknesses

- Trusted binary distribution is not yet complete until a future release is Developer ID signed, notarized, stapled, and distributed with verified checksums. `v1.1.1` is the current source-first hotfix, with an optional unsigned-dev tester artifact only; `v1.1.0` remains a superseded source-first release with a known helper-unreachable issue.
...
- `viftyctl` JSON errors, readiness diagnostics, capabilities run-lifecycle/direct-control metadata, guarded-run lifecycle/force-retry capability checks, missing capability field fail-closed decoding, force retry behavior, run preflight, and Auto-restore reporting are tested.
- `restore-auto` rejects `--idempotency-key` instead of accepting a no-op scoped option; direct restore remains tied to the supervised prepare lifecycle.
...
- Source-first release operations now have Makefile targets for release-note generation, unsigned-dev artifact building, and published source-first readiness checks; unsigned-dev artifact generation is source-ref guarded by default so a zip named for `v<version>` cannot be built from later post-release hardening by accident.
- Local gates pass: `make verify` runs the fast local suite plus warnings-as-errors build, release app bundle, plist lint, and strict codesign verification without installing the app. `make verify-full` and GitHub Actions add the slow evidence/release script suites after the release/readiness evidence schemas, validation report index schema, validation review-result schema, cask trust-metadata, cask checksum handoff, release secret/readiness/source-ref/source-CI/release-workflow/source-first preflights, source-first Source Provenance enforcement, unsigned-dev artifact source-ref guard checks, GitHub Actions Node.js 24 runtime opt-in and cache-action versioning, release status trust documentation, release checklist/source-first note publication, release asset-publication, release TeamID build-wiring, release tag/version identity, release verifier skip-flag, release-summary consistency review, release-summary capture-time consistency, release-checklist evidence collection/review, validation install-provenance capture/review/index checks, validation privacy-review checks including generated summaries, validation capabilities-contract checks, direct validation TSV content checks, review-summary TSV/JSON consistency checks, collector manifest output/status checksum coverage checks, manifest status/output consistency checks, evidence-bundle checksum coverage/recomputation checks, daemon-unavailable capabilities review checks, review-result schema identity checks, malformed review-result indexer, capabilities run-lifecycle/direct-control coverage, guarded-run lifecycle contract checks, guarded-run wrapper-argument preflight checks, guarded-run advertised run-command/workload support checks, guarded-run advertised unavailable-exit checks, guarded-run force-retry capability checks, guarded-run readiness recovery-action checks, guarded-run null-field normalization, capability legacy-field fail-closed decoding, force-retry opt-in, interrupted force-retry JSON error reporting, command-error recovery-action metadata, command-error retry metadata, readiness recovery-action metadata, daemon control-path readiness evidence, missing option-value parse failures, child-command failure classification, guarded-run child executable preflight coverage for PATH and direct paths, workload shortcut coverage, real-process child-command resolution/signal-exit coverage, non-directory executable validation coverage, Auto-restore fan-shape/range preflight coverage before SMC/XPC access, helper install/approve/repair action-copy coverage, signal-handler restoration coverage, fan-control ownership UI summary, developer workload preset, validation-report hardening, safe-agent-cooling guidance, restore-auto scoped-option rejection, support-triage, unsupported-hardware policy, GitHub topic/label metadata checks, community/support standard checks, PR safety review, and CODEOWNERS safety-surface increments.

Remaining local action:

- Use `make verify` as the standard local pre-PR trust gate, and rely on GitHub Actions or local `make verify-full` for the slow evidence/release test suites.
- Keep source-first releases honest: do not retag `v1.1.0` or `v1.1.1`, do not build unsigned-dev assets from later `main` commits, do not rename unsigned-dev assets to the canonical notarized artifact name, and do not promote Homebrew until the future Developer ID lane passes.
...
- Publish concise `viftyctl` examples for Swift, Xcode, npm, pnpm, Bun, Go, cargo, uv, pytest, and generic long-running commands.
...
- `docs/schemas/` gives release tooling, validation review results, validation report indexes, and local agents stable schemas for release readiness, capabilities discovery, required readiness safety fields, read-only audit export, status/lease state, structured command errors, actions, fan telemetry, agent-control status, and check IDs; `viftyctl capabilities --json` now advertises source schema paths, installed bundle schema resource paths, schema IDs, `runLifecycle`, and `directControlLifecycle` guarantees at runtime, and release artifacts are checked for bundled schema resources, valid JSON, and expected schema IDs.
- `examples/viftyctl/guarded-run.sh` provides a shell-friendly preflight plus `viftyctl run --json` wrapper, and `examples/viftyctl/*` now includes tested shortcut scripts for Swift, Xcode, Make, npm, pnpm, Bun, Go, cargo, uv, pytest, local model, and custom workloads, with XCTest coverage proving capabilities `run` command and requested-workload discovery, advertised unavailable-exit handling, `runLifecycle` contract checks, wrapper duration/RPM/reason preflight before contacting Vifty, `supportsForceRetry` checks before passing `--force`, required `recommendedRecoveryAction` handling, missing capability-field fail-closed behavior, ready/degraded/blocked behavior, restore-first decision blocking, missing-decision fail-closed behavior, diagnose failure preservation, opt-in `VIFTY_GUARDED_RUN_FORCE_RETRY` handling, refusal to combine force retry with uncooled fallback, missing and non-executable child-command preflight, structured pre-child failures, executable shortcut scripts, and no raw fan-control commands in those wrappers.
- `guarded-run.sh` now has an explicit `VIFTY_GUARDED_RUN_ALLOW_UNCOOLED=1` path for the case where Vifty blocks cooling but the user wants the child workload to run normally anyway. It still runs read-only capabilities/readiness checks, prints diagnose JSON, refuses to request cooling, and then execs the child directly only for allowed recovery actions; it still refuses uncooled execution for `repairHelper`, `backOffWorkload`, `restoreAutoBeforeRetry`, `inspectPolicy`, `collectHardwareEvidence`, `daemonControlPathReady=false`, and `manualControlActive=true`.
- `viftyctl` rejects unknown wrapper options, duplicate wrapper options, missing option values, and unexpected positional arguments so agent typos fail closed instead of silently changing the intended command.
- `viftyctl diagnose` exits 75 for blocked readiness after printing structured JSON, while validation evidence collection and the guarded-run wrapper both preserve blocked reports correctly. Diagnose JSON also includes `recommendedAgentAction`, `recommendedRecoveryAction`, `safeToRequestCooling`, `daemonControlPathReady`, and `manualControlActive` so agents can distinguish normal, caution, restore-first, manual/user ownership, helper-repair, stop, and safe recovery decisions without parsing warning text.
...
- `scripts/collect-validation-evidence.sh` makes release and hardware reports repeatable without requesting cooling or writing fan state, and now captures bundle plist, installed executable hashes, privacy-review output for likely hostnames, `/Users/...` paths, serial-number labels, or hardware UUID labels, installed release/agent schema resource hashes, advertised schema resource paths from capabilities output, the safe capabilities contract used by guarded workload wrappers and direct prepare/restore flows, optional release-artifact verifier summaries with schema identity, pass/fail, installed-app version matching, skipped/failed check detection, SHA consistency, and artifact-name consistency, optional release checklists with installed-app version matching and required follow-up coverage, LaunchDaemon TeamID, per-binary signing, notarization, and Gatekeeper evidence alongside `viftyctl` JSON plus manifest rows, per-command status files, reviewer-oriented `review-summary.tsv`, and automation-oriented `review-summary.json`; collector tests require each manifest row's stdout, stderr, and status file to exist and appear in `checksums.tsv`, and the reviewer now requires those summary status rows to agree, requires `manifest.tsv` statuses and output files to match captured evidence, requires captured files to be covered by `checksums.tsv`, and recomputes checksum entries against the captured files.
- `make validation-evidence-review` wraps `scripts/review-validation-evidence.sh` for captured evidence bundles in `release`, `supported-hardware`, or `unsupported-hardware` modes and can write schema-backed `review-result.json`, so maintainers can mechanically reject privacy-review findings, missing helper probes, skipped notarization, release summaries with a missing/drifted schema contract, release summaries with skipped/failed checks, checksum mismatches, artifact-name drift, missing or version-drifted release checklists, release checklists without required post-publication follow-up coverage, unsupported-hardware reports that only prove daemon failure rather than a safe hardware block, or failed supported-hardware smoke tests.
- `scripts/summarize-validation-reports.sh` builds JSON/TSV indexes plus an optional conservative `compatibility-matrix.md` draft from valid reviewed `review-result.json` files, rejects missing or drifted validation-review-result schema IDs plus missing `daemonControlPathReady`, missing or unsupported `recommendedAgentAction`, missing or unsupported `recommendedRecoveryAction`, malformed `manualControlActive`, non-read-only review outputs, or release-mode rows with source-first/non-release install sources before they become claims, preserves install provenance, source checksums, `modelFamily`, agent decisions, recovery guidance, agent-run smoke evidence, daemon control-path readiness, and manual-control ownership in compatibility indexes, adds summary counts by model family, validated hardware reports by model family, agent action, recovery action, `safeToRequestCooling`, daemon control-path readiness, and `manualControlActive` for fleet-level safety triage, keeps candidate supported-hardware rows labeled as manual-smoke-required until the review result records `manualSmokeTestResult: "passed-auto-restored"`, then promotes them to `validated-hardware-evidence`; the Markdown draft ignores release-trust rows for hardware status and keeps agent-run smoke separate from manual fan-control validation.
- Hardware-validation guidance now tells reporters to respect `policy.prepareCooldownSeconds` between the manual prepare/restore smoke test and the supervised `viftyctl run` smoke test, and to record a first `PREPARE_RATE_LIMITED` / `retryAfterSeconds` response as cooldown evidence rather than a failed agent-run smoke. Clean current source checkouts should prefer `make agent-run-smoke-evidence-current-build`, which builds `.build/Vifty.app` and runs the smoke through the freshly built `viftyctl`; installed-app source checkouts can still use `make agent-run-smoke-evidence VIFTYCTL=/Applications/Vifty.app/Contents/MacOS/viftyctl`; installed testers can run `/Applications/Vifty.app/Contents/Resources/collect-agent-run-smoke-evidence.sh`; and the raw `scripts/collect-agent-run-smoke-evidence.sh` path remains available for advanced local review. The collector gives supported-hardware reporters a standard captured path for that developer-workload proof: it writes schema-backed `agent-run-smoke-evidence-summary.json` with `schemaID: https://vifty.local/schemas/agent-run-smoke-evidence-summary.schema.json`, manifest/status/checksum files, run stdout/stderr, run proof fields for `coolingLeasePrepared`, `autoRestoreAttempted`, `autoRestoreSucceeded`, `childExitCode`, and basename-only resolved child executable display when advertised, plus follow-up capabilities/status/audit/diagnose evidence only after `pre-capabilities.json` advertises stable capabilities/diagnose/command-error schema IDs, `run`, the `test` workload, and the safe `runLifecycle` contract, including `resolvedChildExecutableReported=true`, and after readiness is safe, including `requestCoolingWithCaution` when `safeToRequestCooling=true`, `daemonControlPathReady=true`, and `manualControlActive=false`; it stops before `viftyctl run` when capabilities are unsafe or readiness is blocked. `make validation-evidence-review VALIDATION_EVIDENCE_AGENT_RUN_SMOKE_SUMMARY=<path>` wraps `scripts/review-validation-evidence.sh`, validates that captured summary, verifies the adjacent smoke bundle manifest rows, per-command status/stdout/stderr files, and checksum coverage/recomputed digests, requires passed captured summaries to report a prepared lease, attempted Auto restore, successful Auto restore, child exit `0`, and matching resolved child executable basename evidence when advertised, and derives `agentRunSmokeResult` without relying on hand-transcribed issue text.
- `scripts/verify-release-artifact.sh` audits a generated release zip before publication and the public cask artifact after publication, verifying checksum, bundle version, required executables, bundled release/validation/agent schema JSON/IDs, plist validity, Developer ID TeamID, LaunchDaemon TeamID allowlist, stapled notarization, and Gatekeeper assessment, with passed or failed JSON summary evidence that declares `https://vifty.local/schemas/release-artifact-summary.schema.json` for release reviewers.
...
- `scripts/check-release-secrets.sh` checks the required GitHub Actions release secret names before a release tag is pushed, so missing Developer ID/notary configuration is visible before the automatic tag-push Release run begins. That first-attempt run is never rerun; after conclusive non-publication, a new patch tag repeats the preflight.
- `scripts/check-release-readiness.sh` now gives maintainers a single read-only human/schema-backed JSON preflight for release metadata, optional required source-ref alignment, release-commit CI status, Release workflow status for the tag, required secret names, and GitHub Release asset presence, so `v1.1.1` can be reported as source-ready but public-release blocked without ambiguity, failed Release runs are distinct from missing assets, and stale release tags can be rejected before promotion.
- `docs/release-status.md` now separates the `v1.1.1` source-first hotfix release, optional unsigned-dev tester artifact, superseded `v1.1.0` helper issue, future trust-complete Developer ID binary release, and Homebrew lane, and tells maintainers not to promote Homebrew until a signed/notarized artifact, checksum, verifier summary, and cask SHA are aligned.
- `scripts/write-release-checklist.sh` now generates either the Developer ID checklist prepended to notarized GitHub Release notes and uploaded as `Vifty-v<version>-release-checklist.md`, or source-first release notes for releases without Apple Developer Program credentials, including a source-provenance section so maintainers record the immutable tag commit and do not confuse later `main` hardening with the published source release. `scripts/build-unsigned-dev-artifact.sh --require-source-ref` and the default Makefile `UNSIGNED_DEV_SOURCE_REF=v<version>` guard keep optional unsigned tester zips tied to the intended source commit.
...
- `ViftyHelper probe` / `probeLocal` now share a tested formatter that includes fan `hardwareMode`, `hardwareModeRawValue`, `hardwareModeKey`, and `targetRPM`, making before/after smoke-test evidence and mode-key casing drift easier to audit.
...
- `scripts/collect-agent-cooling-evidence.sh` gives agent/helper reports a narrow read-only bundle path that captures `capabilities`, `diagnose`, `status`, and bounded `audit` JSON plus exit statuses, launchd/helper install evidence, a manifest, `privacy-review.tsv`, and checksums without requesting cooling, restoring Auto, calling `ViftyHelper`, using `sudo`, or writing SMC keys. When it runs an exact guarded workload with `--guarded-run-preflight`, the bundle records `guarded-run-preflight` as a read-only manifest/summary command alongside `guarded-run-stderr.txt`, `guarded-run-stdout.txt`, and `guarded-run-preflight.status`, so maintainers can review the preflight transcript as first-class evidence without launching the child command. Its privacy review flags likely hostnames, `/Users/...` paths, serial-number labels, and hardware UUID labels before reporters paste agent/helper evidence into public issues.
- `scripts/review-agent-cooling-evidence.sh` reviews those lightweight agent/helper bundles, writes schema-backed `agent-cooling-evidence-review.json` with `schemaID: https://vifty.local/schemas/agent-cooling-evidence-review.schema.json`, accepts blocked diagnose exit `75` as evidence, records a `diagnoseDecision` summary for the diagnose exit status, readiness state, `recommendedAgentAction`, `recommendedRecoveryAction`, `safeToRequestCooling`, `daemonControlPathReady`, `manualControlActive`, display-only `operatorRecoveryCommands`, and `appPreferences.startupMode`, warns when legacy bundles omit startup-mode diagnostics, warns when active manual control is paired with a persisted `Curve` or `Fixed` default, infers missing legacy `daemonControlPathReady` only from structured readiness/recovery fields, records a `capabilitiesDecision` summary for advertised stable capabilities/diagnose/command-error schema IDs, `viftyctl run` support, force-retry discovery, safe run/direct-control lifecycle, metadata limits, daemon status, and unavailable-exit metadata, records an `appInfo` summary for captured app plist status, bundle identifier, short version, and bundle version, treats absent legacy `metadataLimits` as a warning for read-only triage evidence, records `acceptedCommandErrors` only for structured `HELPER_UNREACHABLE` status/audit command errors when blocked diagnose recommends `repairHelper`, requires guarded preflight evidence to keep matching metadata, manifest, summary, stdout, stderr, and status files, distinguishes `guardedRunDecision.captureMode: collectorPreflight` from `captureMode: copiedStderr`, records `collectorPreflight` and `preflightExitStatus` for collector-run `--preflight-only` bundles, and rejects schema drift, manifest/status/checksum drift, privacy findings, missing or contradictory diagnose decision fields, agent-runnable operator recovery commands, unsafe capabilities contract fields, unsafe command errors, or any bundle that says cooling commands were run before maintainers treat the report as evidence.
...
- Keep the lightweight agent/helper support and review path schema-backed through `agent-cooling-evidence-summary.json`, `agent-cooling-evidence-review.json`, `schemaID: https://vifty.local/schemas/agent-cooling-evidence-summary.schema.json`, and `schemaID: https://vifty.local/schemas/agent-cooling-evidence-review.schema.json`.
...
- Ship a future corrected notarized release with checksum, Homebrew cask update, and passing release-artifact verification after Apple Developer Program credentials exist. Do not retag `v1.1.0` or `v1.1.1`; `v1.1.1` is the current source-first hotfix release.
...
- Publish the generated compatibility matrix from validated reports once enough public reports exist.
...
2. Whether the corrected public release should be `1.0.1` or a later patch version. Decision: `v1.1.0` shipped the expanded agent-safety and release-trust surface; `v1.1.1` superseded it as the source-first helper hotfix. The next trusted binary waits for Apple Developer Program credentials.
