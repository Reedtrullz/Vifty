...
**Status:** Active. Post-release local hardening is on `main` after the published `v1.1.0` source tag (`f7d2c636ebf582ac3809998c3fac819d5d87eb72`), and the current local trust gate verifies 461 XCTest cases plus community/support surface checks and release app bundle checks. Public binary trust still depends on a future Developer ID signed/notarized release, Homebrew release metadata, and real Apple Silicon MacBook Pro validation reports.
...
- `examples/viftyctl/guarded-run.sh` provides a shell-friendly preflight plus `viftyctl run --json` wrapper, and `examples/viftyctl/*` now includes tested shortcut scripts for Swift, Xcode, Make, npm, cargo, pytest, local model, and custom workloads, with XCTest coverage proving capabilities `run` command and requested-workload discovery, advertised unavailable-exit handling, `runLifecycle` contract checks, wrapper duration/RPM/reason preflight before contacting Vifty, `supportsForceRetry` checks before passing `--force`, missing capability-field fail-closed behavior, ready/degraded/blocked behavior, restore-first decision blocking, missing-decision fail-closed behavior, diagnose failure preservation, opt-in `VIFTY_GUARDED_RUN_FORCE_RETRY` handling, missing and non-executable child-command preflight, structured pre-child failures, executable shortcut scripts, and no raw fan-control commands in those wrappers.
