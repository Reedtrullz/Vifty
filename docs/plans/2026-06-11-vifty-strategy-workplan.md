...
- `scripts/check-release-readiness.sh` now gives maintainers a single read-only human/JSON preflight for release metadata, required secret names, and GitHub Release asset presence, so `v1.1.0` can be reported as source-ready but public-release blocked without ambiguity.
