...
- This plan reconciliation is documentation-only and must pass the normal exact-head PR check. PR-ready is not public-release-ready: the current-source UI recapture/visual attestation, skipped VoiceOver gate, remediation-build hardware compatibility, independent release reviewer/environment, candidate, signed tag, notarized asset, cask handoff, and installed exact-release proof remain separate and unclaimed.
