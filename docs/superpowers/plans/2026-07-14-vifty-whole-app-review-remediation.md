...
**Status:** Autonomous remediation implementation is present on `codex/vifty-whole-app-remediation` from baseline `21c2f9175dccb9226d6972574070bc21dbede2f0`, but later source changes superseded the last exact-binary UI ledger and the earlier verification snapshots. The worktree remains intentionally uncommitted and unstaged. Current-source compiled/full verification, a fresh 50-row automated UI recapture, a portable checkpoint, and human visual review remain pending. The owner skipped VoiceOver, so that gate remains pending with no VoiceOver behavior claim. No eligible non-owner release reviewer is available, so the protected `release` environment remains absent, the release-manifest candidate remains `null`, and no release was dispatched. The exact public v1.3.2 hardware smoke is recorded separately and does not transfer to the remediation build.
...
- [ ] Make GitHub's `environment: release` scheduling gate authoritative for approval, validate the declaration semantically, and collect repo-settings/API readback proving required reviewers. Keep the provenance script focused on signature, remote tag, ancestry, exact-SHA CI, and version facts.
...
- [ ] `check-release-provenance.sh` must verify: tag signature, immutable remote tag, tag commit is an ancestor of protected `origin/main`, exact source CI success, and version/build/manifest agreement. Environment approval is proven by GitHub scheduling/settings evidence, not inferred by this script.
...
- The UI evidence manifest remains honestly `pending`. A later engineering review found a telemetry-history copy contradiction before the current human Batch 1 was approved, and the source fix changed the hash-bound executable. The prior 50-entry autonomous ledger is historical. The replacement ledger currently has only the new Reduce Transparency row sealed (`1/50`); `--verify-automated` is not yet eligible, human visual review remains `0/28`, and VoiceOver review has not started.
- Immutable tag governance is live: active ruleset `18940029`, `Immutable Vifty release tags`, covers `refs/tags/v*`, blocks update and deletion, and has no bypass actors. The protected `release` environment remains absent (authenticated API readback returned HTTP 404), and the user confirmed that no eligible non-owner release reviewer is currently available. Owner-only approval remains rejected by `scripts/check-release-environment.sh`; the trusted `sign-notarize` job now hashes and repeats that API readback before secrets so GitHub cannot auto-create an unprotected environment and continue. The release manifest candidate remains `null`; no environment, tag, draft, asset, release, or cask mutation occurred.
...
- The single `Quiet` profile remains restored. The protected release environment is still absent, candidate remains `null`, and release dispatch remains blocked because the owner has no independent reviewer; the non-owner gate was not weakened.
...
- Every verification count, product hash, capture ledger, and visual result above is a historical snapshot unless a later exact-source checkpoint explicitly binds it. Current-source compiled/full verification and a fresh 50-row UI recapture remain pending; no current visual or VoiceOver pass is claimed.
