...
When that preflight is safe and a maintainer runs supervised `make agent-run-smoke-evidence`, the saved `agent-run-smoke-evidence-summary.json` uses the same share-safe reason and daemon-path envelope. It preserves daemon identity through SHA-256 fields and path privacy labels rather than raw private workspace paths.
