...
- Current `viftyctl run --json` reports include `resolvedChildExecutable` after the child exits, plus `resolvedChildExecutableSHA256Status` and `resolvedChildExecutableSHA256` when the executable bytes are readable, so agents can audit which executable received cooling and whether byte-level provenance was computed.
