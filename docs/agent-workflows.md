...
The Vifty main window can copy both a short agent rule and common guarded command
templates. Use **Copy Command** when you want an audited wrapper command for
Swift, Xcode, Make, npm, pnpm, Bun, Go, cargo, uv, pytest, local-model, or custom
workload templates; use the read-only preflight entries when an agent only needs
to check readiness without requesting cooling or launching the child command.
...
without hardcoding marker strings. It also includes
`agentCoolingEvidenceCommand` and `agentCoolingPreflightEvidenceCommand` for
read-only support bundles after helper, readiness, restore, or guarded-run
failures; those commands gather evidence only and are not cooling
authorization. Still run `capabilities --json` and `diagnose --json` before any
guarded workload requests cooling. Since the rule contains runnable command examples, live output is
...
- may use `diagnose --json --require-safe` for shell-only gates that should exit `75` when the readiness JSON says a new cooling request is unsafe,
...
For hardware-validation or maintainer triage where you need to know whether the supervised smoke collector may cross the cooling boundary, run `make agent-run-smoke-readiness` first. `AGENT_RUN_SMOKE_READINESS_JSON=1 make agent-run-smoke-readiness` emits `schemaID: https://vifty.local/schemas/agent-run-smoke-readiness.schema.json`, `readOnly: true`, and `coolingCommandsRun: false`; `AGENT_RUN_SMOKE_READINESS_SUMMARY=.build/agent-run-smoke-readiness.json` saves the same JSON for review. The preflight validates daemon-backed capabilities, policy limits, wrapper lifecycle, `safeToRequestCooling`, helper readiness, manual-control state, and optional daemon hash matching without calling `viftyctl run`. Saved summaries omit the full reason text, carry `reasonCharacterCount`, and mark daemon path display values as `system`, `relative`, `basenameOnly`, or `notProvided` so reviewer evidence can stay share-safe. Blocked summaries also copy diagnose `recoverySteps` and display-only `operatorRecoveryCommands` so agents and maintainers can show the next safe human action without scraping human output or running recovery commands automatically.

When that preflight is safe and a maintainer runs supervised `make agent-run-smoke-evidence`, the saved `agent-run-smoke-evidence-summary.json` uses the same share-safe reason and daemon-path envelope. It also records `childCommandName`, `childCommandKind`, `childArgumentCount`, `childArgumentsPrivacy=omitted`, basename-only `run.resolvedChildExecutable`, and `run.resolvedChildExecutablePathPrivacy=basenameOnly` instead of full child arguments or private command paths. The smoke bundle also writes `privacy-review.tsv`; reviewers reject `redaction-needed` rows before public hardware reports are accepted. It preserves daemon identity and executable identity through SHA-256 fields and path privacy labels rather than raw private workspace paths.
