...
2. Confirm the read-only release readiness preflight:

   ```sh
   scripts/check-release-readiness.sh --version <version> --repo Reedtrullz/Vifty
   ```

   This validates local release metadata, checks source CI for the release commit, checks required release secret names, and inspects the GitHub Release asset list. Its JSON output declares `schemaID: https://vifty.local/schemas/release-readiness.schema.json` so release evidence can reject drift. Before the tag exists, this may report a missing source-CI/tag check; before publication it may still report a missing GitHub Release. After publication it should pass only when source CI passed for the tag commit and the zip, checksum, artifact summary, and release checklist assets are present.
...
11. After publication, run `scripts/check-release-readiness.sh --version <version> --repo Reedtrullz/Vifty --json` again and keep the passed schema-backed JSON with release notes or validation evidence. The final JSON should show `source-ci`, `release-secrets`, and `github-release` all passed.
