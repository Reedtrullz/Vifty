...
`childExitCode`, and, when advertised by capabilities, `resolvedChildExecutable`
...
Keep the captured `viftyctl-run.json` alongside the summary. In current builds it also records `resolvedChildExecutable` after the child exits, allowing maintainers to audit which absolute executable received the bounded cooling lease.
