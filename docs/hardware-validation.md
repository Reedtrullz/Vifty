...
```sh
make agent-run-smoke-readiness
...
make validation-evidence-review \
  ...
  VALIDATION_EVIDENCE_MANUAL_SMOKE_RESULT=passed-auto-restored \
  VALIDATION_EVIDENCE_MANUAL_SMOKE_SOURCE=<hardware-validation-issue-url> \
  VALIDATION_EVIDENCE_AGENT_RUN_SMOKE_READINESS_SUMMARY=.build/agent-run-smoke-readiness.json \
  VALIDATION_EVIDENCE_AGENT_RUN_SMOKE_SUMMARY=.build/vifty-agent-run-smoke-<timestamp>/agent-run-smoke-evidence-summary.json \
  VALIDATION_EVIDENCE_REVIEW_SUMMARY=.build/vifty-validation-<timestamp>/review-result.json
```

Run `make agent-run-smoke-readiness` before the collector. In JSON mode (`AGENT_RUN_SMOKE_READINESS_JSON=1 make agent-run-smoke-readiness`) the result is read-only evidence with `schemaID: https://vifty.local/schemas/agent-run-smoke-readiness.schema.json`, `readOnly: true`, and `coolingCommandsRun: false`; it only runs `viftyctl capabilities --json` and `viftyctl diagnose --json`. Use `AGENT_RUN_SMOKE_READINESS_SUMMARY=.build/agent-run-smoke-readiness.json` to save the same JSON as a durable preflight artifact. The saved summary omits the full reason text, records `reasonCharacterCount`, marks daemon path display values as `system`, `relative`, `basenameOnly`, or `notProvided` so public evidence does not need local absolute paths, and carries diagnose `recoverySteps` plus display-only `operatorRecoveryCommands` when blocked. If it exits `75`, do not run `make agent-run-smoke-evidence`; clear the listed blockers first. For clean current-source proof, `make agent-run-smoke-readiness-current-build` builds `.build/Vifty.app`, runs the preflight through the freshly built `viftyctl`, and blocks if the installed LaunchDaemon helper does not match the freshly built daemon.

The agent-run smoke readiness summary declares `schemaID: https://vifty.local/schemas/agent-run-smoke-readiness.schema.json`. When supplied through `VALIDATION_EVIDENCE_AGENT_RUN_SMOKE_READINESS_SUMMARY`, the reviewer records `agentRunSmokeReadinessSource` and verifies that the preflight was read-only, ran no cooling commands, matched `viftyctl-diagnose.json` for the shared readiness fields and display-only `operatorRecoveryCommands`, and, when ready, had daemon-backed policy/run-lifecycle capability evidence with no blockers. The reviewer rejects readiness summaries whose `operatorRecoveryCommands` are missing, malformed, or marked agent-runnable. The agent-run smoke evidence summary declares `schemaID: https://vifty.local/schemas/agent-run-smoke-evidence-summary.schema.json`. It uses the same share-safe reason and daemon-path envelope as the readiness summary: `reason` is `omitted-for-privacy`, `reasonCharacterCount` preserves length evidence, `reasonPrivacy` is `omitted`, and daemon path display values carry privacy labels. It also records the child command as `childCommandName`, `childCommandKind`, `childArgumentCount`, and `childArgumentsPrivacy=omitted`, with `childCommand[]` reduced to the basename display value only. When `preflight.resolvedChildExecutableReported=true`, the summary records the resolved child executable as a basename-only `run.resolvedChildExecutable` with `run.resolvedChildExecutablePathPrivacy=basenameOnly`; the captured raw `viftyctl-run*.json` file keeps the absolute executable path for local review. The reviewer validates that schema identity and derives `agentRunSmokeResult` / `agentRunSmokeSource` from the captured file only after checking the adjacent smoke bundle: `manifest.tsv` must match the summary `commands[]`, each command's stdout/stderr/status file must exist and match its recorded status, `privacy-review.tsv` must have status 0 with no `redaction-needed` rows, and `checksums.tsv` must cover and recompute the summary, manifest, command stdout/stderr, status files, and privacy-review files. It also copies the reviewed privacy command into `review-result.json` as `agentRunSmokePrivacyReviewSource` and `agentRunSmokePrivacyReviewStatus` so the generated index can show the privacy-review proof instead of hiding it inside the bundle. When `rateLimitRetry.attempted=true`, the reviewer also requires the initial `viftyctl-run` JSON to be structured `PREPARE_RATE_LIMITED` cooldown evidence with `safeToProceed=false`, matching `retryAfterSeconds`, no lease prepared, no Auto restore attempted, and a nonzero exit status that matches the recorded `rateLimitRetry.initialExitStatus`; the final `run` proof must reference the `viftyctl-run-retry` stdout/stderr/status files. A passed captured summary must also report `coolingLeasePrepared=true`, `autoRestoreAttempted=true`, `autoRestoreSucceeded=true`, `childExitCode=0`, and, when `preflight.resolvedChildExecutableReported=true`, a basename-only `resolvedChildExecutable` whose value matches the basename of the captured final `viftyctl-run*.json` absolute executable path. When present, `childTerminationReason`, `childSignal`, `childSignalName`, and `resolvedChildExecutableSHA256Status` must also match the final run JSON; `computed` requires a lowercase SHA-256 `resolvedChildExecutableSHA256`, while `unavailable` means the exact executable path is known in the raw run JSON but byte-level hashing was not available. That makes developer-workload proof include the bounded lease, Auto-restore outcome, child exit/termination classification, share-safe executable identity, privacy-review clearance, and explicit digest availability rather than only the wrapper exit status. For passed `local-ad-hoc-build` smoke, the reviewer also requires `daemonRuntime.matchRequired=true`, `installedDaemonPresent=true`, and `matchesExpectedDaemon=true` in the captured smoke bundle, or the same match through `VALIDATION_EVIDENCE_AGENT_RUN_SMOKE_READINESS_SUMMARY` when only issue-template text is available, so current-build smoke cannot overclaim a stale installed helper daemon. When the captured smoke summary includes `preflight.appPreferences`, the reviewer copies it into `review-result.json` as `agentRunSmokeStartupMode`, `agentRunSmokeStartupModeSource`, and `agentRunSmokeStartupModeReadError`, and the generated compatibility index preserves those fields for filtering blocked/manual-control reports. These fields explain recovery context only; they are not cooling authorization. If only issue-template text is available, use `VALIDATION_EVIDENCE_AGENT_RUN_SMOKE_RESULT=passed-auto-restored VALIDATION_EVIDENCE_AGENT_RUN_SMOKE_SOURCE=<hardware-validation-issue-url>#agent-run-smoke VALIDATION_EVIDENCE_AGENT_RUN_SMOKE_READINESS_SUMMARY=<path-to-ready-json>`.
...
The index reads `review-result.json` files only and writes schema-backed JSON with `schemaID: https://vifty.local/schemas/validation-report-index.schema.json`. It requires each input review result to declare `schemaID: https://vifty.local/schemas/validation-review-result.schema.json`, then rejects malformed review results, non-read-only review results, review results that declare cooling commands ran, unsupported modes/statuses, unsupported install-source values, release-mode rows whose install source is source-first or otherwise not release-capable, invalid or missing required source SHA/checksum fields, mutable or missing source refs for source-build tag evidence, missing `daemonControlPathReady`, missing or unsupported `recommendedAgentAction`, missing or unsupported `recommendedRecoveryAction`, malformed `manualControlActive`, malformed `failedCheckIDs` / `coolingBlockerIDs`, malformed `agentRunSmokePrivacyReviewStatus`, non-empty `coolingBlockerIDs` paired with `safeToRequestCooling=true`, and contradictory passed results with failures. Valid index rows can show release-trust evidence, unsupported-hardware safe-block evidence, supported-hardware candidate evidence, and `validated-hardware-evidence` rows when the review result includes `manualSmokeTestResult: "passed-auto-restored"`, while preserving `installSource`, `sourceRef`, `sourceSHA`, `sourceArtifactName`, `sourceArtifactSHA256`, `modelFamily`, `recommendedAgentAction`, `recommendedRecoveryAction`, `failedCheckIDs`, `coolingBlockerIDs`, `daemonControlPathReady`, `manualControlActive`, `agentRunSmokeResult`, `agentRunSmokeSource`, `agentRunSmokeReadinessSource`, `agentRunSmokePrivacyReviewSource`, `agentRunSmokePrivacyReviewStatus`, `agentRunSmokeStartupMode`, `agentRunSmokeStartupModeSource`, and `agentRunSmokeStartupModeReadError` for compatibility-table filtering. Legacy review results that predate `manualControlActive`, blocker-ID arrays, agent-run readiness provenance, privacy-review provenance, or agent-run startup-mode diagnostics are indexed as blank/unknown rather than inferred. `modelFamily` is derived from the model identifier prefix, for example `MacBookPro18`; use the summary fields `countsByModelFamily` and `validatedHardwareReportsByModelFamily` to group reviewed reports without manually scanning every row. The optional `--output-markdown` file is a conservative compatibility matrix draft generated from hardware rows only; it ignores release-trust rows for hardware status, keeps agent-run smoke separate from validated-hardware claims, labels rows as validated only when manual smoke evidence passed, and surfaces blocker IDs plus agent-run privacy-review status in the evidence columns when reviewed evidence contains them. The JSON summary also includes `countsByRecommendedAgentAction`, `countsByRecommendedRecoveryAction`, `countsBySafeToRequestCooling`, `countsByDaemonControlPathReady`, and `countsByManualControlActive` so maintainers can spot stop-before-cooling, helper-repair, active/manual-control, and unsafe-readiness reports without scanning every row.
...
4. Before touching Fixed or Curve smoke, run the read-only manual-smoke preflight:
   ...
   For current-source validation, prefer `make manual-smoke-readiness-current-build`; it builds `.build/Vifty.app`, runs the freshly built `viftyctl`, and blocks if the installed LaunchDaemon helper does not match the freshly built daemon. Use `MANUAL_SMOKE_READINESS_SUMMARY=.build/manual-smoke-readiness.json MANUAL_SMOKE_READINESS_JSON=1 make manual-smoke-readiness-current-build` to save the ready JSON for review. Installed-app testers without a source checkout can run `/Applications/Vifty.app/Contents/Resources/check-manual-smoke-readiness.sh --viftyctl /Applications/Vifty.app/Contents/MacOS/viftyctl --json --summary manual-smoke-readiness.json`. For automation or issue triage, use `MANUAL_SMOKE_READINESS_JSON=1 MANUAL_SMOKE_READINESS_SUMMARY=<path> make manual-smoke-readiness`. Pass that JSON back to the reviewer as `VALIDATION_EVIDENCE_MANUAL_SMOKE_READINESS_SUMMARY=<path>` for local-ad-hoc manual smoke. The preflight only runs `viftyctl diagnose --json`, may hash the installed LaunchDaemon helper and expected daemon when asked, records `schemaID: https://vifty.local/schemas/manual-smoke-readiness.schema.json`, `readOnly: true`, and `coolingCommandsRun: false` in JSON mode, also records `daemonRuntime`, ordered `recoverySteps`, and display-only `operatorRecoveryCommands`, exits `75` when smoke must be skipped, and lists blockers such as `installed daemon does not match expected build daemon`, `manual control active before manual smoke`, `daemonControlPathReady=false`, unsupported hardware, missing fans/sensors, or critical thermal pressure.
...
can tell which privileged daemon handled the run from SHA-256 evidence without
needing private absolute paths. The smoke summary also omits the full reason
text, records `reasonCharacterCount`, marks daemon path display values as
`system`, `relative`, `basenameOnly`, or `notProvided`, records only the child command basename, argument count, and resolved executable basename, and records
`installSource`, `sourceRef`, `sourceSHA`, and optional source artifact fields; this target sets
...
`coolingLeasePrepared`, `autoRestoreAttempted`, `autoRestoreSucceeded`,
`childExitCode=0`, `childTerminationReason`, and, when advertised by capabilities, basename-only `resolvedChildExecutable`
plus `resolvedChildExecutablePathPrivacy=basenameOnly` in the summary; on successful child runs these are derived from
...
Keep the captured `viftyctl-run.json` alongside the summary. In current builds it also records the raw absolute `resolvedChildExecutable` after the child exits and inferred signal fields when the exit code looks like `128 + signal`, allowing maintainers to audit which executable received the bounded cooling lease and whether the workload appears to have been interrupted while the public summary stays basename-only.
