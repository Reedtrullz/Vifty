...
Run `make agent-run-smoke-readiness` before the collector. In JSON mode (`AGENT_RUN_SMOKE_READINESS_JSON=1 make agent-run-smoke-readiness`) the result is read-only evidence with `schemaID: https://vifty.local/schemas/agent-run-smoke-readiness.schema.json`, `readOnly: true`, and `coolingCommandsRun: false`; it only runs `viftyctl capabilities --json` and `viftyctl diagnose --json`. If it exits `75`, do not run `make agent-run-smoke-evidence`; clear the listed blockers first. For clean current-source proof, `make agent-run-smoke-readiness-current-build` builds `.build/Vifty.app`, runs the preflight through the freshly built `viftyctl`, and blocks if the installed LaunchDaemon helper does not match the freshly built daemon.
