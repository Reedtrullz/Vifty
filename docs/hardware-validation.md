...
`system`, `relative`, `basenameOnly`, or `notProvided`, records only the child command basename and argument count, and records
...
`childExitCode`, `childTerminationReason`, and, when advertised by capabilities, `resolvedChildExecutable`
in the summary; on successful child runs these are derived from
...
Keep the captured `viftyctl-run.json` alongside the summary. In current builds it also records `resolvedChildExecutable` after the child exits and inferred signal fields when the exit code looks like `128 + signal`, allowing maintainers to audit which absolute executable received the bounded cooling lease and whether the workload appears to have been interrupted.
