...
final class HardwareSnapshotProbeFormatterTests: XCTestCase {
    func testProbeOutputIncludesFanHardwareModeRawValueAndTargetRPM() {
        ...
        XCTAssertTrue(output.contains("fan[0] name=\"Left Fan\" rpm=3200 min=1400 max=6000 controllable=true hardwareMode=Forced hardwareModeRawValue=1 targetRPM=5000"))
    ...
    func testProbeOutputMarksMissingFanHardwareTelemetryExplicitly() {
        ...
        XCTAssertTrue(output.contains("fan[1] name=\"Right Fan\" rpm=2200 min=1500 max=7200 controllable=true hardwareMode=unknown hardwareModeRawValue=nil targetRPM=nil"))
    }
}
