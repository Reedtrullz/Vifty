...
final class AppSourceRegressionTests: XCTestCase {
    ...
    func testCurveEditorShowsDraggableChartForActualFanCurves() throws {
        ...
        XCTAssertTrue(contentView.contains("Text(\"\\(Int(temperature.rounded())) C\")"))
        XCTAssertTrue(contentView.contains("Text(\"\\(Int(rpm.rounded()).formatted(.number.grouping(.automatic))) RPM\")"))
