...
final class DocumentationTrustSurfaceTests: XCTestCase {
    ...
    func testReleaseStatusKeepsHomebrewInstallClaimsHonest() throws {
        ...
        XCTAssertTrue(releaseStatus.contains("the latest published public release is `v1.1.1`, a source-first hotfix release because the project did not have active Apple Developer Program credentials at publication time."))
        ...
        XCTAssertTrue(releaseStatus.contains("Developer ID account setup status as of 2026-07-07: the intended personal Apple Developer team is visible but still pending."))
        XCTAssertTrue(releaseStatus.contains("Pending Apple Developer Program access is not release evidence."))
        XCTAssertTrue(releaseStatus.contains("Do not use another organization's team or certificate for Vifty unless that organization is intentionally meant to own Vifty's public signing identity"))
        XCTAssertTrue(releaseStatus.contains("Trusted notarized Developer ID release:** unavailable until the intended Developer ID account is active, the release secrets are configured for that team, and the signed/notarized artifact verifier passes."))
        ...
        XCTAssertTrue(releaseStatus.contains("Before Apple activates the intended personal team, the only allowed Developer ID prep work is non-secret and non-promotional:"))
        ...
        XCTAssertTrue(releaseStatus.contains("Prepare the secret names and local commands, but do not store certificate material in the repo or Obsidian."))
    ...
    func testCompetitiveAnalysisCodifiesDifferentiatedRoadmap() throws {
        ...
        XCTAssertTrue(analysis.contains("Homebrew parked until a Developer ID signed, notarized release exists"))
        XCTAssertTrue(analysis.contains("unsigned-dev builds labeled as tester convenience only"))
