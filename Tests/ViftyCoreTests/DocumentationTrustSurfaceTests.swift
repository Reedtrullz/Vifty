...
final class DocumentationTrustSurfaceTests: XCTestCase {
    ...
    func testReadmeLinksCompatibilityAndHardwareValidation() throws {
        ...
        XCTAssertTrue(readme.contains("[v1.4.8 MacBookPro18,1 report](docs/validation-reports/2026-09-13-v1.4.8-macbookpro18-supported/review-result.json)"))
    ...
    func testReleaseStatusKeepsHomebrewInstallClaimsHonest() throws {
        ...
        XCTAssertTrue(readme.contains("The exact installed public `v1.4.8` build 16 also passed release-mode review and supervised Fixed → Temperature Curve → Auto validation on `MacBookPro18,1`"))
        XCTAssertTrue(readme.contains("The historical v1.3.2 report remains separately scoped to that older binary"))
    ...
    func testCompatibilityPageRequiresReportBackedEvidence() throws {
        ...
        XCTAssertTrue(compatibility.contains("| M1 Pro/Max MacBook Pro | Validated evidence for `MacBookPro18,1` / v1.4.8 only |"))
        ...
        XCTAssertTrue(matrix.contains("| MacBookPro18 | Validated hardware evidence | 2 | 4 | 1 | 0 | 3 |"))
        ...
        XCTAssertTrue(index.contains("\"validatedHardwareReports\": 2"))
    ...
    func testDocsKeepTelemetryHistoryVisualizationLocalAndScoped() throws {
        ...
        let trustModel = try read("docs/trust-model.md")
        let analysis = try read("docs/competitive-analysis.md")
        let plan = try read("docs/plans/2026-09-13-post-v1.4.8-validation.md")
        let agents = try read("AGENTS.md")
        ...
        XCTAssertTrue(analysis.contains("Do not compete as a general-purpose system monitor yet."))
        XCTAssertTrue(analysis.contains("full historical monitoring, cloud sync, analytics, and persistent telemetry remain out of scope without a separate privacy plan"))
        XCTAssertTrue(plan.contains("Keep telemetry local and in-memory"))
        XCTAssertTrue(plan.contains("do not add accounts, analytics, cloud sync, or persistent rolling history"))
    ...
    func testNextWorkplanPrioritizesAvailableHardwareAndTrustBoundaries() throws {
        let plan = try read("docs/plans/2026-09-13-post-v1.4.8-validation.md")
        let historicalPlan = try read("docs/plans/2026-06-13-next-workplan.md")
        let analysis = try read("docs/competitive-analysis.md")
        let agents = try read("AGENTS.md")

        XCTAssertTrue(analysis.contains("[plans/2026-09-13-post-v1.4.8-validation.md](plans/2026-09-13-post-v1.4.8-validation.md)"))
        XCTAssertTrue(agents.contains("`docs/plans/2026-09-13-post-v1.4.8-validation.md`"))
        XCTAssertTrue(plan.contains("exact published v1.4.8 binary"))
        XCTAssertTrue(plan.contains("MacBookPro18,1"))
        XCTAssertTrue(plan.contains("Fixed RPM, Temperature Curve, and final Auto restoration"))
        XCTAssertTrue(plan.contains("make validation-evidence"))
        XCTAssertTrue(plan.contains("make manual-smoke-readiness"))
        XCTAssertTrue(plan.contains("make validation-evidence-review"))
        XCTAssertTrue(plan.contains("passed-auto-restored"))
        XCTAssertTrue(plan.contains("Keep telemetry local and in-memory"))
        XCTAssertTrue(plan.contains("Do not replace it with v1.4.8 by string substitution"))
        XCTAssertTrue(historicalPlan.contains("Historical plan, superseded on 2026-09-13"))
        XCTAssertTrue(historicalPlan.contains("available real hardware for immediate validation is the user's M1 Pro MacBook Pro"))
        XCTAssertTrue(historicalPlan.contains("make validation-evidence-current-build"))
        XCTAssertTrue(historicalPlan.contains("VALIDATION_EVIDENCE_MANUAL_SMOKE_RESULT=passed-auto-restored"))
    }
    ...
    func testHistoricalStrategyWorkplanRemainsTraceable() throws {
        let workplan = try read("docs/plans/2026-06-11-vifty-strategy-workplan.md")

        XCTAssertTrue(workplan.contains("Historical strategy record, superseded on 2026-09-13"))
        ...
        XCTAssertTrue(workplan.contains("community/support standard checks"))
