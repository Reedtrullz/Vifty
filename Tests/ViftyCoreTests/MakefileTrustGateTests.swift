...
final class MakefileTrustGateTests: XCTestCase {
    func testVerifyTargetRunsLocalTrustGates() throws {
        ...
        XCTAssertTrue(makefile.contains("VALIDATION_EVIDENCE_INSTALL_SOURCE ?= local-ad-hoc-build"))
        XCTAssertTrue(makefile.contains("VALIDATION_EVIDENCE_SOURCE_REF ?= main"))
        XCTAssertTrue(makefile.contains("VALIDATION_EVIDENCE_SOURCE_SHA ?= $(shell git rev-parse HEAD 2>/dev/null)"))
    ...
    func testVerifyTargetIsListedAsPhonyAndHelpVisible() throws {
        ...
        XCTAssertTrue(makefile.contains(".PHONY: app install pkg validation-evidence agent-cooling-evidence agent-cooling-evidence-review agent-run-smoke-evidence source-first-release-notes unsigned-dev-artifact source-first-readiness clean-app clean-pkg test verify help clean"))
        XCTAssertTrue(makefile.contains("verify: ## Run local trust gates without installing"))
        XCTAssertTrue(makefile.contains("validation-evidence: ## Collect read-only release/hardware validation evidence"))
