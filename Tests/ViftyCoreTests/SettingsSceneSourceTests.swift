...
final class SettingsSceneSourceTests: XCTestCase {
    func testSettingsLiveInNativeSceneAndUsesNativeTabs() throws {
        ...
        XCTAssertTrue(settingsView.contains("SettingsGeneralView(model: model, softwareUpdates: softwareUpdates)"))
        ...
        XCTAssertTrue(general.contains("Section(\"Updates\")"))
        XCTAssertTrue(general.contains("\"Automatically check for updates\""))
        XCTAssertTrue(general.contains("softwareUpdates.performPrimaryAction()"))
        let updatesSection = try XCTUnwrap(general.range(of: "updatesSection"))
        let startupSection = try XCTUnwrap(general.range(of: "Section(\"Startup\")"))
        let appearanceSection = try XCTUnwrap(general.range(of: "Section(\"Appearance\")"))
        let loginSection = try XCTUnwrap(general.range(of: "Section(\"Login\")"))
        XCTAssertLessThan(updatesSection.lowerBound, startupSection.lowerBound)
        XCTAssertLessThan(startupSection.lowerBound, appearanceSection.lowerBound)
        XCTAssertLessThan(appearanceSection.lowerBound, loginSection.lowerBound)
