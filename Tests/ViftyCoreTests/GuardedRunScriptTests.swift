...
final class GuardedRunScriptTests: XCTestCase {
    ...
    func testGuardedRunRejectsCapabilitiesWithMissingCommandErrorSchemaID() throws {
        let harness = try ScriptHarness(
            state: "ready",
            capabilitiesCommandErrorSchemaID: nil
        )
    ...
    func testGuardedRunRejectsDiagnoseCommandErrorWithDriftedSchemaID() throws {
        let harness = try ScriptHarness(
            state: "ready",
            diagnoseExitCode: 1,
            commandErrorOverride: #"{"schemaVersion":1,"schemaID":"https://example.invalid/viftyctl-command-error.schema.json","command":"diagnose","safeToProceed":false,"message":"daemon unavailable"}"#
        )
        ...
        XCTAssertEqual(result.exitCode, 75)
        XCTAssertTrue(result.stderr.contains("diagnose command-error schema identity is not recognized"), result.stderr)
        XCTAssertTrue(result.stderr.contains("viftyctl-command-error.schema.json"), result.stderr)
        XCTAssertTrue(result.stderr.contains("\"schemaID\":\"https://example.invalid/viftyctl-command-error.schema.json\""), result.stderr)
        XCTAssertFalse(FileManager.default.fileExists(atPath: harness.logURL.path))
    }

    func testGuardedRunRejectsDiagnoseReadinessWithDriftedSchemaVersion() throws {
        let harness = try ScriptHarness(
            state: "ready",
            diagnoseSchemaVersion: 2
        )
        ...
        XCTAssertEqual(result.exitCode, 75)
        XCTAssertTrue(result.stderr.contains("diagnose readiness schema version is not recognized"), result.stderr)
        XCTAssertTrue(result.stderr.contains("expected schemaVersion=1"), result.stderr)
        XCTAssertFalse(FileManager.default.fileExists(atPath: harness.logURL.path))
    }
    ...
    func testGuardedRunCanRunChildWithoutViftyCoolingWhenUserAllowsUncooledFallback() throws {
        ...
        XCTAssertEqual(decision["decisionReason"] as? String, "uncooledFallbackAllowed")
        XCTAssertEqual(decision["exitCode"] as? Int, 0)
    ...
    func testGuardedRunBlocksBeforeRunWhenManualControlIsActiveEvenIfPayloadClaimsSafe() throws {
        let harness = try ScriptHarness(
            state: "degraded",
            recommendedAction: "requestCooling",
            recommendedRecoveryAction: "restoreAutoBeforeRetry",
    ...
    func testGuardedRunDecisionMarksBlockedUncooledFallback() throws {
        ...
        XCTAssertEqual(decision["uncooledFallbackRequested"] as? Bool, true)
        XCTAssertEqual(decision["uncooledFallbackAllowed"] as? Bool, false)
        XCTAssertEqual(decision["decisionReason"] as? String, "coolingBlockersPresent")
    ...
    func testGuardedRunTreatsNullStateAsDiagnoseFailureWhenDiagnoseFails() throws {
        let harness = try ScriptHarness(
            state: "ready",
            diagnoseExitCode: 1,
            commandErrorOverride: #"{"schemaVersion":1,"schemaID":"https://vifty.local/schemas/viftyctl-command-error.schema.json","state":null,"command":"diagnose","safeToProceed":false,"message":"daemon unavailable"}"#
        )
    ...
    func testWorkloadExampleScriptsDelegateThroughGuardedRun() throws {
        let cases: [(script: String, arguments: [String], expected: [String])] = [
            ...
            (
                ...
                ["--", "--mode=production"],
                ["run", "--json", "--workload", "build", "--duration", "25m", "--max-rpm-percent", "75", "--reason", "pnpm build", "--", "pnpm", "build", "--", "--mode=production"]
            ),
            (
                "examples/viftyctl/pnpm-test.sh",
                ["--", "--runInBand=false"],
                ["run", "--json", "--workload", "test", "--duration", "20m", "--max-rpm-percent", "70", "--reason", "pnpm test", "--", "pnpm", "test", "--", "--runInBand=false"]
            ),
            (
                "examples/viftyctl/bun-build.sh",
                ["--minify"],
                ["run", "--json", "--workload", "build", "--duration", "25m", "--max-rpm-percent", "75", "--reason", "bun run build", "--", "bun", "run", "build", "--minify"]
            ),
            (
                "examples/viftyctl/bun-test.sh",
                ["--timeout", "10000"],
                ["run", "--json", "--workload", "test", "--duration", "20m", "--max-rpm-percent", "70", "--reason", "bun test", "--", "bun", "test", "--timeout", "10000"]
            ),
            (
            ...
            (
                ...
                ["--out-dir", "dist"],
                ["run", "--json", "--workload", "build", "--duration", "25m", "--max-rpm-percent", "75", "--reason", "uv build", "--", "uv", "build", "--out-dir", "dist"]
            ),
            (
                "examples/viftyctl/uv-test.sh",
                ["Tests"],
                ["run", "--json", "--workload", "test", "--duration", "20m", "--max-rpm-percent", "70", "--reason", "uv pytest", "--", "uv", "run", "pytest", "Tests"]
            ),
            (
...
private final class ScriptHarness {
    ...
    private func writeFakeChildTools() throws {
        for tool in ["swift", "xcodebuild", "make", "npm", "pnpm", "bun", "go", "cargo", "python3", "uv", "local-model-runner", "custom-runner"] {
