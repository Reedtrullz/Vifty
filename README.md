...
The exact public zip and the checked-in cask both resolve to SHA-256 `92e803fa372c9cbd387b9b903ae0531031bc244804946d441616760f1d04a61f`. Independent `scripts/verify-release-artifact.sh --team-id X88J3853S2` checks pass for bundle version, required executables and schemas, Developer ID TeamID, LaunchDaemon allowlisting, stapling, and Gatekeeper without skip flags. Installed release-mode validation also passes for the exact public `v1.3.0` app with a matching signed helper, no Vifty manual-control marker, no cooling blockers, and no cooling commands. `v1.1.1` remains the published source-first fallback. Explicit Auto-restoration and manual Fixed/Curve hardware compatibility remain separate reviewed claims; see [docs/release-status.md](docs/release-status.md).
...
The Homebrew cask now points at the published `v1.3.0` notarized zip with SHA-256 `92e803fa372c9cbd387b9b903ae0531031bc244804946d441616760f1d04a61f`. `scripts/verify-release-artifact.sh --team-id X88J3853S2` passes against that exact public artifact using the cask checksum, and read-only installed release-mode review passes with matching signed helper parity and no cooling commands. This proves release, cask, and installed-binary trust alignment; it is not explicit Auto-restoration or manual hardware compatibility proof. Keep compatibility claims gated by [docs/release-status.md](docs/release-status.md), and never point the cask at an unsigned-dev artifact.
