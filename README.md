...
`capabilities --json` returns the supported commands, supported workload names, source schema paths, installed bundle schema resource paths, stable schema IDs, shell exit-code meanings, `runLifecycle` guarantees for `viftyctl run`, and daemon policy limits such as max lease duration, allowed RPM percent range, and prepare cooldown. If the daemon status cannot be read, it still prints the static command contract with `daemonStatusAvailable: false`, `policySource: "fallbackUnavailable"`, a disabled fallback policy, and exits with `exitCodes.unavailable`. Rate-limited decisions include `retryAfterSeconds` so agents can wait without parsing human text.
