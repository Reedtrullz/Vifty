...
The checked-in [v1.4.8 MacBookPro18,1 report](docs/validation-reports/2026-09-13-v1.4.8-macbookpro18-supported/review-result.json) records `manualSmokeTestResult: "passed-auto-restored"` for the exact public build 16 after supervised Fixed → Temperature Curve → Auto validation. That is validated hardware evidence for this binary and model identifier, not a broad Apple Silicon or current-branch claim. The historical [v1.3.2 report](docs/validation-reports/2026-07-14-v1.3.2-macbookpro18-supported/review-result.json) remains valid for that older binary only. Separately, `docs/validation-reports/2026-06-18-macbookpro18-main-agent-run-smoke/review-result.json` records a local-ad-hoc `agentRunSmokeResult: "passed-auto-restored"`; it remains developer-workload proof for that older build and does not establish agent-run compatibility for v1.4.8. Guarded workload wrappers also require capabilities to advertise audited `workloadTemplates` with the expected title/workload/duration/RPM cap/reason/child-command defaults for every shortcut wrapper before they request cooling, so older or drifted `viftyctl` payloads cannot rely on copied defaults by convention only.
...
The exact public zip and the checked-in cask both resolve to SHA-256 `7853ca7ad8ca51a35aa89f56f2779c3d2b528523b7e3198be7c550d208e16683`. The publication workflow summary and independent cask verification record that this exact artifact passed bundle-version, required-executable and bundled-schema, Developer ID TeamID, LaunchDaemon allowlist, stapling, and Gatekeeper checks without skip flags. The exact installed public `v1.4.8` build 16 also passed release-mode review and supervised Fixed → Temperature Curve → Auto validation on `MacBookPro18,1`; see the [release review](docs/validation-reports/2026-09-13-v1.4.8-macbookpro18-release/review-result.json) and [hardware attestation](docs/validation-reports/2026-09-13-v1.4.8-macbookpro18-supported/manual-smoke-attestation.md). The historical v1.3.2 report remains separately scoped to that older binary. `v1.1.1` remains the published source-first fallback; see [docs/release-status.md](docs/release-status.md).
...
The Homebrew cask now points at the published `v1.4.8` notarized zip with SHA-256 `7853ca7ad8ca51a35aa89f56f2779c3d2b528523b7e3198be7c550d208e16683`. The publication-time workflow and independent cask verifier recorded a pass for that exact artifact, and the exact installed build 16 has now passed release review plus supervised Fixed/Curve/Auto compatibility on `MacBookPro18,1`. Keep compatibility claims gated by [docs/release-status.md](docs/release-status.md), and never point the cask at an unsigned-dev artifact.
