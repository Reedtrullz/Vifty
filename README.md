...
- Optional starter-rule discovery: run `viftyctl agent-rule --json`, require `schemaID: "https://vifty.local/schemas/viftyctl-agent-rule.schema.json"` and `capabilities.schemaIDs.agentRule`, then treat the rule as read-only guidance, not cooling authorization.
