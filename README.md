...
Current-source screenshot pending. The [historical README image](docs/images/vifty-screenshot.png) predates the current remediation and is not current UI evidence, so it is intentionally not displayed here. Replace it only with the checkpoint-bound hero from a fresh exact-source UI recapture.
