...
- Run `viftyctl diagnose --json` before cooling; require diagnose readiness schema version `1`, `safeToRequestCooling: true`, `daemonControlPathReady: true`, `manualControlActive: false`, and, when `daemonRuntime.matchRequired` is true, `daemonRuntime.matchesExpectedDaemon: true`. If diagnose exits nonzero with a command-error payload, require the command-error schema identity advertised by capabilities before trusting recovery fields.
