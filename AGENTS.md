...
- `scripts/check-release-readiness.sh` — read-only public-release preflight that validates release metadata, required secret names, and GitHub Release asset presence, with schema-backed JSON output for release evidence.
...
- `swift test` runs `ViftyCoreTests` (373 tests).
