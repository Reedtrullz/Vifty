...
- `scripts/check-release-readiness.sh` — read-only public-release preflight that validates release metadata, optional required source-ref alignment, source CI for the release commit, Release workflow status for the tag, required secret names, and GitHub Release asset presence, with schema-backed JSON output for release evidence.
...
- `swift test` runs `ViftyCoreTests` (385 tests).
