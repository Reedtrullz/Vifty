...
19. **Separate source-first from trusted binaries** — `v1.1.1` remains the immutable source-first hotfix and `v1.1.0` is superseded; neither may be retagged or refreshed from later source. `v1.3.1` is the published Developer ID release; its exact public artifact, cask checksum, release evidence, notarization, stapling, TeamID, and Gatekeeper checks passed. Signed-helper parity, installed release-mode review, explicit Auto restoration, and manual Fixed/Curve hardware compatibility remain separate claims for the exact `v1.3.1` binary; prior-version proof does not transfer. Do not flatten source-first, local-build, or prior-version proof into current trusted-binary claims. Trusted binary releases use `.github/workflows/release.yml`.
