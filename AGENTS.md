...
- `Sources/ViftyCore/Models.swift` — All data types: Fan, TemperatureSensor, HardwareSnapshot, FanCurve, FanMode, FanCommand, ControlState, ViftyError.
- `Sources/ViftyCore/HardwareService.swift` — `HardwareService` protocol + `FanControlCoordinator` actor + `ManualControlMarker`.
- `Sources/ViftyCore/RealMacHardwareService.swift` — `RealMacHardwareService` (daemon-first SMC reads, local fallback).
...
- `Sources/Vifty/AppModel.swift` — `@MainActor ObservableObject` driving the UI polling loop.
...
3. **Daemon-first** — `RealMacHardwareService` tries `ViftyDaemonClient` first; falls back to direct SMC if the daemon is unreachable.
...
- `FanControlCoordinatorTests` uses `FakeHardware` (actor + `HardwareService`).
- `FanCurveTests` tests interpolation, clamping, SMC float encode/decode.
