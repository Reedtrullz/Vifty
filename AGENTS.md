...
- `swift test` runs `ViftyCoreTests` (417 tests).
...
- `ValidationEvidenceReviewScriptTests` tests `scripts/review-validation-evidence.sh` accepts supported-hardware, unsupported-hardware safe-block, and installed-release evidence bundles only when the captured files support those claims, accepts daemon-unavailable capabilities status when static contract evidence still passes, writes pass/fail `review-result.json` summaries with explicit manual smoke-test evidence, and rejects privacy-review findings, missing helper probes, capabilities contract drift, skipped notarization evidence, release-summary schema drift, release-summary checksum/artifact/check-status drift, missing or version-drifted release checklists, or failed supported-hardware smoke tests.
