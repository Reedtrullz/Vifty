...
- `.github/workflows/release.yml` — protected-main, manually dispatched signed-tag workflow for Developer ID signing, notarization, stapling, checksums, and GitHub Release publishing.
...
- `scripts/check-release-secrets.sh` — verifies required GitHub Actions release secret names are configured on the protected `release` environment before pushing/rerunning release tags; reads environment-secret names only, never values, and fails closed if the environment is absent or unreadable.
