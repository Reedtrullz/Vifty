...
19. **Separate source-first from trusted binaries** — `v1.1.1` remains the immutable source-first hotfix and `v1.1.0` is superseded; neither may be retagged or refreshed from later source. `v1.2.0` is the published Developer ID release; its exact public artifact, cask checksum, release evidence, notarization, stapling, TeamID, Gatekeeper, signed-helper parity, Auto restoration, and installed release-mode review passed. Manual Fixed/Curve hardware compatibility remains a separate claim. Do not flatten source-first or local-build proof into trusted-binary claims. Trusted binary releases use `.github/workflows/release.yml`.
