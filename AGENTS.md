...
- `swift test` runs `ViftyCoreTests` (394 tests).
...
- `ViftyCtlProcessRunnerTests` tests child executable resolution, non-executable/missing command rejection before run, shell-style exit-code mapping for real child signal exits, and signal-handler restoration after handled child runs.
