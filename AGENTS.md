...
19. **Separate source-first from trusted binaries** — `v1.1.1` remains the immutable source-first hotfix and `v1.1.0` is superseded; neither may be retagged or refreshed from later source. `v1.2.0` is a Developer ID candidate until its exact public artifact, cask checksum, release evidence, notarization, stapling, and Gatekeeper verification pass. Do not flatten candidate or source-first proof into trusted-binary claims. Trusted binary releases use `.github/workflows/release.yml`.
