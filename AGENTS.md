...
- `scripts/check-release-readiness.sh` — read-only public-release preflight that validates release metadata, required secret names, and GitHub Release asset presence, with JSON output for release evidence.
...
- `swift test` runs `ViftyCoreTests` (372 tests).
